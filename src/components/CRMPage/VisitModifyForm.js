...
function VisitModifyForm(props) {
  const [memo, setMemo] = useState("");
  const [loading, setLoading] = useState(false);

  const memoHandler = (event) => setMemo(event.target.value);

  const submitHandler = (event) => {
    event.preventDefault();

    if (memo === "") return alert("간략한 고객 메모를 적어주세요.");

    setLoading(true);

    let visitInfo = {
      userId: props.modifyData.userId,
      memo: memo,
    };
    ...
    axios
      .put(API + "/crm/customer/memo/{clientId}", formData, {
        headers: {
...
export default VisitModifyForm;
